...
function onCreatePost()
    ...
    makeLuaText('kadeEngineWatermark', songName .. ' - ' .. difficultyName, 0, 4, getProperty('healthBarBG.y') + 50)
...
function onUpdate()
    if displayAcc then
        if hits < 1 and misses < 1 then
            ...
            setTextString('newScoreTxt', 'Score:' .. score .. ' | Combo Breaks:' .. misses .. ' | Accuracy:' .. round(rating * 100, 2) .. ' % | (' .. ratingFCKade .. ') ' .. ratingNameKade)
        end
    else
        if hits < 1 and misses < 1 then
